...
**配色改成黑白**（`commonvars.h`）：标题/专辑名/正文/箭头全白，
下划线、按钮框、文字阴影全黑。两套默认值都改，因为屏幕颜色到底取
`_PALETTE` 还是 `_PIC` 取决于渲染路径（上游把两套刻意设成同色）。
`_PIC` 三者**必须互不相同**，否则 `command_line_parsing.c` 会把整套重置。

**文字阴影**画在**高亮层**，与下划线同色：

| 动画层 | 内容 | 颜色 |
|---|---|---|
| `impic` | 标题、专辑名、曲名、`Next`/`Previous`/`Menu` | 白 |
| `hlpic` | 下划线、索引页按钮框、**上述所有文字的阴影** | 黑 |

为什么阴影必须在另一层：子画面整幅只有 4 个调色板项，而 spumux 的
`s->pal[]` 是从**图像层各像素自己的颜色**取的 —— 同一层里再现第二种
颜色要么被合并、要么直接报 `Too many colors in base picture`。
详见 `docs/TROUBLESHOOTING.md` 第 23 节。

偏移 = `TEXT_SHADOW_DX/DY`（2 px，字号 25~30 时刚好不糊）。

⚠️ 标题（光盘标题）的阴影要**追加到 `command1`**，不能在
`prepare_overlay_img()` 里直接改 `hlpic` 文件 —— `generate_menu_pics()`
紧接着会 `copy_file(impic, hlpic)` 把它覆盖掉。
