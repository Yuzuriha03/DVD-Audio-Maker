...
16. [选曲菜单（AMG / ASVS）的坑（26 个小节）](#16-选曲菜单amg--asvs的坑)
17. [诊断手法速查](#17-诊断手法速查)
...
- 每个 cell 的 `start` 在同一项里写**两次**，`end` 只写**一次**
- 正确的链要求 `start_{j+1} == end_j + 1`

⇒ 凡是「恰好出现 2 次」的值就是某个 start，则 **`start - 1` 必须也能在表里
找到**（它是上一个 cell 的 end）。找不到就是链断了。

实测（旧成品盘2，5 页）：

```
start(恰好 2 次) = [40, 81, 123, 165]
前驱缺失        = [81, 123, 165]        → ✗ 失败
```

已用旧构建的数据验证过它会报错（排除「自检永远通过」）。
