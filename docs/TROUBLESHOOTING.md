...
| 操作 | 观察 |
|---|---|
| **自动连播**进到下一首 | 曲目号**显示正确**，且**音频确实前进** |
| 但在此状态下按「下一段」| **回到第 1 首**，曲目号变 `0/56` |
| 同样状态下按「上一段」| **能回到上一首**（曲目号正确），再按一次就**没反应**了 |
| **直接跳转**（用菜单 / 列表选曲）| 音频**从头 0:00 开始**，曲目号显示 `0/56` |
| 从跳转状态按「下一段」| 回到第 1 首 |
| 「跳转至」列表本身 | **曲目号正确**（1..56），选哪首就跳哪首 |

关键区别：**自动连播时播放器「当前曲目」的状态是对的**（能前进、能后退），
但**只要一次跳转（或用了一次上一段/下一段）就会掉回「未定位在曲目内」**，
之后所有基于「当前曲目」的操作都退化。
...
- **「当前位置 → 第几首」这条反向追踪在「自动连播」路径下是通的**，但
  **一次跳转之后就失效**（掉回 0）。所以不是「播放器完全没有这个机制」，
  而是**它需要的信息在跳转路径上取不到** —— 最可能的载体仍是
  `ATS_PTT_SRPT`（16.31）：PTT 表给出每个曲目的区间，
  播放器靠它把「当前时间」映射回「第几首」。
  直接跳转时它只能把位置设到「时间 0:00 且曲目号未定义」，于是显示 0/56。
  这也解释了为什么**跳转后总是从头（0:00）开始** —— 它没有「该曲目起点在
  时间轴上的位置」可用，只能回到时间轴原点。
...
**现象（用户报告）**：播放时只在换专辑时刷新封面，同一张专辑内切曲目不换图。

### 根因：ATS 静图表的两个偏移字段被写成了常量

ATS 的静图表（title 描述符 `+14`）布局是：
...
- `止 = 6×轨数 + 10×图数 - 1` —— 只有**末轨**碰巧正确

于是同一 title 内所有轨都声称「我的图在 `[6n, 6n+10p-1]`」→
播放器逐轨查表时取到的都是**第 1 张图**，曲目内自然不换图。

### 对比：封面正常的商业盘是按轨递进的

```
李娜 轨1: 01 00 00 48 00 51   a=72 =6×12+10×0   b=81 =a+9
李娜 轨2: 01 00 00 52 00 5b   a=82 =6×12+10×1   b=91 =a+9
巴赫 轨1: 01 04 00 6c 00 75   a=108=6×18+10×0   b=117=a+9

本工程（修前）轨1..n: 01 00 00 24 00 5f   a=36（常量）  b=95（常量）
```

通用式：`a(r) = 6×轨数 + 10×(前 r 轨的图数之和)`，
`b(r) = a(r) + 10×本轨图数 - 1`。每轨恰好 1 张图时退化为 `a = 6n + 10r`、`b = a + 9`。

> `byte1` **不要动**：源码里 `0x04` 对应 `--stilloptions manual`
> （可手动翻页的幻灯片）。上游本来就只在传了该选项时才置 0x04，
> 本工程从不传，实测一直是 `0x00`。

### 验证手法（可复用）

**要知道「哪些字节变了」，先提取修复前后两份 `ATS_01_0.IFO` 再逐字节 diff**，
并把差异位置映射回各个 title 的静图表区间：

```python
diff = [i for i in range(len(A)) if A[i] != B[i]]        # A=新, B=旧
# 逐个 title 算出静图表区间 [st, st+6*ntracks)，检查 diff 是否全落在里面
```

实测：**78 个差异字节 = 2×(56 轨 − 17 title)**，**全部落在静图表内**；
其余 6 个系统文件（`AUDIO_PP.IFO` / `AUDIO_SV.IFO/VOB` / `AUDIO_TS.IFO/BUP/VOB`）
与修复前**逐字节相同**。

那个 `2×(轨数−title数)` 不是巧合：每轨 2 个字段，但
**首轨的 `a` 和末轨的 `b` 本来就对**，故每 title 少了 2 个字段；
又因数值都 < 256，只有低字节变化 → 恰好 2 字节/字段。

### 教训

**把几个补丁一起停用（回滚）时，每个都要单独确认理由。**
...
## 21. ImageMagick `-repage` 是**算子**，不加括号会毁掉整页

拼缩略图网格用的是「逐层 `-repage +x+y` 定位，最后 `-flatten`」：

```bash
magick -size 720x576 xc:black \
  \( 封面1.jpg -resize 104x104^ -extent 104x104 -repage +38+5 \) \
  \( 封面2.jpg -resize 104x104^ -extent 104x104 -repage +218+5 \) \
  -flatten 页.jpg
```

`-repage` **必须写在 `( )` 里面**。它是 IM 的**算子**（operator）而不是
设置项 —— 不加括号时会对「当前图像列表里的**每一张**」生效，包括开头那张
720x576 的黑底色。黑底被挪到最后一格的位置后，画布露出 `-flatten` 的
默认**白底**：

```
症状：整页只有右下角一张封面，其余全白
      （那道白不是封面，是 flatten 的默认底色）
      整页均值 238（正常 ~86）
```

`magick` **不报错**，退出码 0，只能靠量像素发现：

```bash
magick 页.jpg -format "%[fx:mean*255]\n" info:      # 238 → 可疑
# 逐格量封面均值，找哪几格是空的
for i in 0 1 2 3; do
  magick 页.jpg -crop 104x104+$((38+i*180))+5 +repage \
    -format "%[fx:mean*255]\n" info:
done
```

同一个坑踩了**两次**，第二次是加专辑名时写成了
`... caption:名字 ) -repage +5+111`（括号提前闭合）。

**教训**：往 IM 命令里拼 `( )` 时，`-repage` / `-rotate` / `-resize` 这类
算子的归属位置要逐字核对 —— 对比旁边那段的写法，两段的 `)` 位置必须一致。

---

## 22. `mogrify` 命令串末尾少了空格 → **静默什么都不画**（返回 0）

拼 mogrify 命令时，各段都是 `strcat` 到同一个缓冲，最后再 `strcat` 输出
文件名。**若前一段末尾没有空格，两者会粘成一个参数**：
...
* 把最后那个「参数」当成坐标串的一部分，**没有输出文件**
* 默认把结果写到 **stdout**（被日志吞掉）
* 文件一个像素都不变
* **退出码 0** → 上层 `system(...) == -1` 的判断放过

症状：**某几页的按钮框 / 翻页箭头完全不出现，而构建日志一切正常**
（按钮数自检也只查 XML 里的编号，查不到画面）。

实测这次是索引页中招：12 个描边框和翻页箭头全没画，专辑页却正常 ——
因为 `mogrify_img()` 的格式串末尾本来就有空格，自己写的两个函数漏了。

**查法**（别靠肉眼）：

```bash
# 每页都有 impic<N>.png（文字层）和 hlpic<N>.png（高亮层 = 文字层 + 按钮框）
identify -format "%[fx:mean.a*w*h]\n" impic0.png hlpic0.png
```

`hlpic` 的墨迹**必须多于** `impic`。两者相等 = 那条命令静默失效。
`02_build.py` 的 `check_menu_overlay()` 现在把这个不变量做成构建期自检。
