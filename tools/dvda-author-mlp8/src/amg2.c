...
uint32_t create_topmenu(char *audiotsdir, command_t *command, globalData *globals)
{
  ...
  /* 一级菜单（专辑索引页）的**画面**在这里现画，直接写成
     `<tempdir>/bgpic<p>.jpg` —— 紧接着的 generate_background_mpg() 就会去
     读它们。必须在这个位置：compute_menu_pages() 先填好 `page_ntracks`
     （每页放几张专辑），而命令行解析阶段还没有这个数据。 */
  if (img->index_pages)
    dvda_make_index_pages(img, globals);
  ...
  switch (globals->topmenu)
    ...
    case RUN_GENERATE_PICS_SPUMUX_DVDAUTHOR:
      ...
      generate_menu_pics(command, img, m_ngroups, m_ntracks, globals);
                    ...
                    uint32_t *relative_sector_pointer_VTSI, globalData *globals)
{
  ...
  /* 菜单区视频属性（制式）由 AUDIO_TS.VOB 自检得出，
               ...
               uint8_t *numtitles, uint8_t **ntitletracks, uint64_t **titlelength, globalData *globals)
{
  ...
  /* 菜单区视频属性（制式）由 AUDIO_TS.VOB 自检得出（上游硬编码 0x53=PAL），
     见 docs/DVDA-AUTHOR-CHANGES.md。 */
