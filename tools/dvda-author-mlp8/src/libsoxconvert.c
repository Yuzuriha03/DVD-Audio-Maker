...
int soxconvert(char *input, char *output, globalData *globals)
{
  ...
  check(in = sox_open_read(input, NULL, NULL, NULL));
  stage++;

  /* Open the output file; we must specify the output signal characteristics.
   * Since we are using only simple effects, they are the same as the input
   * file characteristics */
  check(out = sox_open_write(output, &in->signal, NULL, NULL, NULL, NULL));
  stage++;
  ...
  check(sox_add_effect(chain, e, &in->signal, &in->signal) == SOX_SUCCESS);
  stage++;
