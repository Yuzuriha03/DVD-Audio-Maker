                              ...
                              globalData *globals)
{
  ...
  *pts_ptr = (P[0] >> 1 & 0x3) << 30 | P[1] << 22
  ...
  if (globals->logdecode)
    fprintf(aob_log, "INF;PTS d�code;%08X ;;;;\n", *pts_ptr);
}
                                    ...
                                    globalData *globals)
{
  ...
  RW_FIELD(scr_bytes)
  decode_SCR(scr_bytes, SCRint_ptr, globals);
                                          ...
                                          uint32_t DTS, globalData *globals)
{
  switch (mlp_flag)
    {
    case  0xC0:
      PES_header_data_length[0] = 0xA;
      break;

    case  0:
      PES_header_data_length[0] = 0;
      break;
  ...
  else if (mlp_flag == 0x80) // write only if PCM (even at PTS == 0)
    {
  ...
  if (*DTS_ptr)
    {
      ...
      decode_pts(PTS_DTS_data, DTS_ptr, globals);
    }
  else
    {
      RW_FIELD(PTS_data);
      decode_pts(PTS_data, PTS_ptr, globals);
    }
  ...
  if (info->type == AFMT_MLP)
    {
      ...
      fwrite(unknown_mlp, 2, 1, fp);
      /* offset_count += */   fwrite(sample_rate, 1, 1, fp);   // 0
      /* offset_count += */   fwrite(unknown2, 1, 1, fp);
    }
  else
    {
      /* offset_count += */   fwrite(downmix_index, 1, 1, fp); // PCM only
      /* offset_count += */   fwrite(sample_size, 1, 1, fp);   // PCM only
      /* offset_count += */   fwrite(sample_rate, 1, 1, fp);   // PCM only
      /* offset_count += */   fwrite(unknown2, 1, 1, fp);
    }
                                   ...
                                   globalData *globals)
{
  ...
  else if (sample_size[0] == 0x00 || sample_size[0] == 0x22)
    {
  ...
  if (pack_in_title == 0)
  ...
  else if (bytesinbuffer < info->lpcm_payload)
    {
      ...
      if (info->type != AFMT_MLP)
        ...
        {
          write_pes_padding(fp, 2048 - (ftello(fp) % 2048), globals);
        }
    }
  else
    {
                    ...
                    globalData *globals)
{
  ...
  /***       +14     ***/
  read_pack_header(fp, &SCR, globals);

  /***       +18  if first  ***/
  if (position == FIRST_PACK) read_system_header(fp, globals);
...
read_audio_pes_header at pack %" PRIu32 ";Disk/computed;%d;%d\n",
      ...
      pts_calc = calc_PTS(info, pack_in_title, &cumbytes, globals);
    }
  else
    {
...
int decode_ats(globalData *globals)
{
  FILE *fp;
               ...
               globalData *globals)
{
  ...
  allocate_mlp_tracktable(&files[i], globals);
