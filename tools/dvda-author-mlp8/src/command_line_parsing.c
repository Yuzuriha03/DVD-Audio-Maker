...
separated by a colon, with rgb components by commas");
        ...
        case 43:
          /* 一级菜单（专辑索引页）：前 N 页是缩略图网格。
             页上的「曲目」是专辑格子，按钮是 `jump menu`。
             见 menu.c 的 compute_menu_pages 与 xml.c 的分支。 */
          img->index_pages = (uint8_t) atoi(optarg);
          foutput(ANSI_COLOR_MAGENTA "[PAR]" ANSI_COLOR_RESET
                  "  First %d menu screen(s) are album index pages.\n",
                  (int) img->index_pages);
          break;

        case 44:
          /* 索引页每格的封面图清单 —— **一个文件，每行一个路径**，
             顺序 = 「页序 × 格子序」（第 p 页第 k 格 = 第
             p * INDEX_PER_PAGE + k 行）。C 按 `page_ntracks[p]` 取用。

             ⚠️ 为什么不是逗号分隔的列表：专辑目录名里**有 ASCII 逗号**
             （实测 `奔流,因你不息(游戏《鸣潮》原声音乐) - Single`），
             用 `,` 分会把一条路径切成两条。`--screentext` 早就有这个
             毛病（标签里不能出现 `=`/`:`/`,`）。改用行分隔后，
             唯一不能出现的字符是换行 —— 而文件名里本来就没有换行。

             画面本身由 C 现画（menu.c 的 dvda_make_index_pages()），
             这里只提供素材路径。 */
          if (img->indexcovers)
            {
              foutput("%s\n", ERR "Index covers already specified,"
                      " skipping...");
              break;
            }
          {
            char *listpath = make_absolute(optarg);
            FILE *lf = fopen(listpath, "r");
            if (!lf)
              EXIT_ON_RUNTIME_ERROR_VERBOSE("Cannot read the album cover"
                                            " list file")

            int cap = 64, n = 0;
            img->indexcovers = calloc(cap, sizeof(char *));
            if (!img->indexcovers)
              EXIT_ON_RUNTIME_ERROR_VERBOSE("Out of memory")

            char line[CHAR_BUFSIZ];
            while (fgets(line, sizeof(line), lf))
              {
                size_t len = strlen(line);
                while (len && (line[len - 1] == '\n' || line[len - 1] == '\r'))
                  line[--len] = '\0';
                if (!len) continue;                    /* 跳过空行 */

                if (n == cap)
                  {
                    cap *= 2;
                    char **g = realloc(img->indexcovers, cap * sizeof(char *));
                    if (!g) EXIT_ON_RUNTIME_ERROR_VERBOSE("Out of memory")
                    img->indexcovers = g;
                  }
                img->indexcovers[n++] = strdup(line);
              }
            fclose(lf);
            img->indexcoverssize = n;

            foutput(ANSI_COLOR_MAGENTA "[PAR]" ANSI_COLOR_RESET
                    "  %d album cover(s) for the index pages (from %s).\n",
                    n, listpath);
            free(listpath);
          }
          break;
  ...
  switch (globals->topmenu)
    ...
    case RUN_MJPEG_GENERATE_PICS_SPUMUX_DVDAUTHOR :
      /* 非索引页各取自己那张背景图。
         ...
         ⚠️ **索引页在这里被跳过** —— 它们的画面由
         `dvda_make_index_pages()` 现画，而那个函数要等
         `compute_menu_pages()` 填好 `page_ntracks` 才能跑，
         也就是在 amg2.c 里、本阶段之后。

         ⚠️ `--background` 的语义因此是**只覆盖非索引页**：
         第 u 页（u >= index_pages）用 `backgroundpic[u - index_pages]`。
         没有索引页时（默认）就是原来的 1:1 对应，行为不变。
         原式还固定用 [0]，使逗号列表失效（所有页共用第 1 张）—— 已改。 */
      for (u = 0; u < (int) img->nmenus; ++u)
        {
          if (u < (int) img->index_pages) continue;   /* 已由 C 画好 */

          char name[16];
          sprintf(name, "%s%d%s", "bgpic", u, ".jpg");

          int bidx = u - (int) img->index_pages;
          const char *srcpic =
            (bidx < globals->backgroundpicsize && img->backgroundpic[bidx])
            ? img->backgroundpic[bidx] : img->backgroundpic[0];
