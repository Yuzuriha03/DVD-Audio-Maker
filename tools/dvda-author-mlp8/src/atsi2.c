...
int get_afmt(fileinfo_t *info,
             audioformat_t *audioformats,
             int *numafmts,
             globalData *globals)
{
  int i;
  ...
  if (*numafmts == 9)
    {
      EXIT_ON_RUNTIME_ERROR_VERBOSE("DVD-Audio discs cannot manage more "
                                    "than 8 different audio formats per group.\n"
...
static inline uint16_t downmix_coeff(float db)
{
  ...
  else if (db <= 4.3)    c = (uint16_t)(0x15 - (4.3 - db) / 0.2) ;
  else if (db <= 7.2)    c = (uint16_t)(0x23 - (7.2 - db) / 0.2) ;
  else if (db <= 10.1)   c = (uint16_t)(0x31 - (10.1 - db) / 0.2) ;
  else if (db <= 13.2)   c = (uint16_t)(0x40 - (13.2 - db) / 0.2) ;
  else if (db <= 16.1)   c = (uint16_t)(0x4E - (16.1 - db) / 0.2) ;
  else if (db <= 19)     c = (uint16_t)(0x5C - (19 - db) / 0.2) ;
  else if (db <= 22.1)   c = (uint16_t)(0x6B - (22.1 - db) / 0.2) ;
  else if (db <= 25)     c = (uint16_t)(0x79 - (25 - db) / 0.2) ;
  else if (db <= 27.9)   c = (uint16_t)(0x87 - (27.9 - db) / 0.2) ;
  else if (db <= 31)     c = (uint16_t)(0x96 - (31 - db) / 0.2) ;
  else if (db <= 33.9)   c = (uint16_t)(0xA4 - (33.9 - db) / 0.2) ;
  else if (db <= 36.8)   c = (uint16_t)(0xB2 - (36.8 - db) / 0.2) ;
  else if (db <= 39.7)   c = (uint16_t)(0xC0 - (39.7 - db) / 0.2) ;
  else if (db <= 41.2)
    {
      if (db == 40.1) c = 0xC8;
      else if (db == 40.5) c = 0xC9;
      else if (db == 40.9) c = 0xCA;
      else
        c = (uint16_t)(0xC7 - (41.2 - db) / 0.2) ;
    }
  else if (db <= 42.9)   c = (uint16_t)(0xCF - (42.9 - db) / 0.4) ;
  else if (db <= 61.8)   c = (uint16_t)(0xFE - (61.8 - db) / 0.4) ;
  else if (db == 100.0) c = 0xFF; // off

  return c;
}

int create_atsi(command_t *command,
                char *audiotsdir,
                uint8_t titleset,
                uint8_t *atsi_sectors,
                uint16_t *ntitlepics,
                globalData *globals)
{
...
#define db command->db
  ...
  while (j < ntracks)
    {
      ...
      get_afmt(&files[j], audioformats, &numafmts, globals);
  ...
  for (j = 0; j < numafmts; ++j)
    {
      // TODO: CHECK this as 0X1 was observed (MLP) for menuless disc
      // (3/16/96 DW, G1T3, 2/24/44 DVDA-C).
          ...
          for (int w = 0; w < 16; ++w)
            {
              ...
              // Left front speaker channel to left stereo speaker

              atsi[i + 2 + w * 18]  =  downmix_coeff(db[w].Lf_l);

              // Left front speaker channel to right stereo speaker

              atsi[i + 3 + w * 18]  =  downmix_coeff(db[w].Lf_r);

              // Right front speaker channel to left stereo speaker

              atsi[i + 4 + w * 18]  =  downmix_coeff(db[w].Rf_l);

              // Right front speaker channel to right stereo speaker

              atsi[i + 5 + w * 18]  =  downmix_coeff(db[w].Rf_r);

              // Center speaker channel to left stereo speaker

              atsi[i + 6 + w * 18]  =  downmix_coeff(db[w].C_l);

              // Center speaker channel to right stereo speaker

              atsi[i + 7 + w * 18]  =  downmix_coeff(db[w].C_r);

              // Left surround or Surround speaker channel to left stereo speaker

              atsi[i + 8 + w * 18]  =  downmix_coeff(db[w].S_l);

              // Left surround or Surround speaker channel to left stereo speaker

              atsi[i + 9 + w * 18]  =  downmix_coeff(db[w].S_r);

              // Right surround to left stereo speaker

              atsi[i + 10 + w * 18] =  downmix_coeff(db[w].Rs_l);

              // Right surround to right stereo speaker

              atsi[i + 11 + w * 18] =  downmix_coeff(db[w].Rs_r);

              // Low-frequency effects (subwoofer) to left stereo speaker

              atsi[i + 12 + w * 18] =  downmix_coeff(db[w].LFE_l);

              // Low-frequency effects (subwoofer) to right stereo speaker

              atsi[i + 13 + w * 18] =  downmix_coeff(db[w].LFE_r);
  ...
  // [200806] The number numtitles must be equal to number of audio
  // zone titles plus video zone titles linked to. Gapless tracks are
  // packed in the same title.
  ...
  for (j = 0; j < numtitles; ++j)
    {
      ...
      // MLP looks like 0x0101, when multichannel, 0x0001 when stereo
      // or mono, PCM unfirmly 0x0100
  ...
  for (j = 0; j < numtitles; ++j)
    {
      ...
      // Pointer to PTS table

      uint16_copy(&atsi[i], 0x0010);
      i += 2;

      // Pointer to sector table

      uint16_copy(&atsi[i], 16 + 20 * ntitletracks[j]);
      i += 2;
      ...
      if (img->count || img->stillvob || img->active)
        ...
        // Pointer to a stillpic data table

        uint16_copy(&atsi[i], 16 + 32 * ntitletracks[j]);
      ...
      for (t = 0; t < ntitletracks[j]; ++t)
        {
          // These seem to be pointers to a lookup table in the first
          // sector of the ATSI

          x = get_afmt(&files[k], audioformats, &numafmts, globals);
          x = (x * 8) << 8;
          ...
          //  0x10 for stereo or mono // or if no downmix table, so
          //  means: "no downmix"

          if (files[j].channels < 2 || files[k + t].downmix_table_rank == 0)
            {
              x |= 0x0010;
            }
          else
            {
              ...
              //0x0 to 0x0F, rank of downmix table (0-based)

              x |= files[k + t].downmix_table_rank - 1;
            }
      ...
      if (img->count || img->stillvob || img->active)
        {
          ...
          if (globals->veryverbose)
            foutput(MSG_TAG "pictitlecount=%d for ntitlepics[%d]=%d\n",
                    pictitlecount,
                    j,
                    ntitlepics[j]);
          ...
          for (r = 0; r < ntitletracks[j]; ++r)
            {
              ...
              /* 本循环是「一轨一条记录」，**不能**因本 title 没有独立图片就
          ...
          for (t = 0; t < ntitletracks[j]; ++t)
            {
              ...
              if (img->npics[trackcount - 1] == 0)
                {
                  if (globals->debugging)
                    {
                      foutput(INF "Skipping track with no pics,"
                              " t=%d, trackcount=%d\n", t, trackcount);
                    }
                  continue;
                }
              ...
              // while inside a track pic list or if beginning a track
              // that has pics in it

              while (pictrackcount < img->npics[trackcount - 1])
                {
                  ...
                  if ((img->options)
                  ...
                  else
                    {
                      ...
                      if ((img->options == NULL)
                          || (img->options[s] == NULL)
                          || (img->options[s]->manual == 0))

                        uint32_copy(&atsi[i], pictrackcount * 90000);
                    }
  ...
  *atsi_sectors = (uint8_t) ((i + 2047) / 2048);
  ...
  uint32_copy(&atsi[12], files[ntracks - 1].last_sector + 2 * *atsi_sectors);
  ...
  nb_atsi_files += create_file(audiotsdir, basename, atsi,
                               2048 * (*atsi_sectors), globals);
